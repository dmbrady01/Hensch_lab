%Simulation of Multisensory Integration Data - Change Modulation (maxmod)
%
...
maxmod = [2 6 20]; %maximal modulation V2(realistic = 6, other = 10); V1(realistic = 4, other = 10)
...
s = RandStream('swb2712','Seed',1394); %Starts a random number generator at a particular seed
RandStream.setDefaultStream(s)

%% Preparing modulation

%%%%%%%%%%%%%%%%%%%%%%%%%% PREPARING MODULATION %%%%%%%%%%%%%%%%%%%%%%%%%%
mod = cell(1,length(maxmod));  
for i = 1:length(maxmod)
    mod{i} = minmod + (maxmod(i)-minmod)*rand(nneurons,1);  %generates a column specifying the degree of modulation for each neuron
end
...
SM = cell(1,length(maxmod)); %Single stimulus condition
theoCM = cell(1,length(maxmod)); %Theoritical combined condition
CM = cell(1,length(maxmod)); %'Measured' combined condition
...
parfor i = 1:length(maxmod)
    SM{i} = mean(poissrnd(lambda,nneurons,ntrials),2); %mean of ntrials in response to the single stimulus at a firing rate of lambda
...
MI = cell(1,length(maxmod)); %'Measured' MI
theoMI = cell(1,length(maxmod)); %'Theoritcal' MI
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% FIGURING OUT AXES %%%%%%%%%%%%%%%%%%%%%%
x_width = cell(1,length(maxmod));
for i = 1:length(maxmod)
    x_width{i} = [minmod-1 maxmod(i)-1]; %Limits of x-axis
end

y_min = ones(length(maxmod),1);
y_max = ones(length(maxmod),1);
for i = 1:length(maxmod) %Figures out the max and min y values for MI under each lambda condition
...
for i = 1:length(maxmod)
    subplot(length(maxmod),3,3*i-2)
    hold on
    scatter(theoMInoinf{i},MInoinf{i})
    line((minmod-1):(maxmod(i)-1),(minmod-1):(maxmod(i)-1)) %Line to orient if theoritical is equal to measured
    axis([x_width{i} y_width]); %sets axis
    text(-0.5,.75*y_width(2),['Maximal modulation factor: ' num2str(maxmod(i))]) %label of firing rate
    switch i
        case round(median(find(maxmod>0)))
...
x_width = cell(1,length(maxmod));
for i = 1:length(maxmod)
    x_width{i} = [minmod maxmod(i)]; %Limits of x-axis
end
...
for i = 1:length(maxmod)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%  PLOTTING RESIDUALS %%%%%%%%%%%%%%%%%%%%%
    subplot(length(maxmod),3,3*i-1)
    hold on
    scatter(modnoinf{i},MInoinf{i}-theoMInoinf{i})
    line(minmod:maxmod(i),zeros(length(minmod:maxmod(i)),1)) %Line to orient if theoritical is equal to measured
    axis([x_width{i} y_width]); %sets axis
    switch i
        case round(median(find(maxmod>0)))
    ...
    %%%%%%%%%%%%%%%%%%%%%%%  HIST OF RESIDUALS %%%%%%%%%%%%%%%%%%%%%
    subplot(length(maxmod),3,3*i)
    hold on
    hist(MInoinf{i}-theoMInoinf{i},30)
    ylim = get(gca,'YLim');
    line([0 0],[0 max(ylim)])
    title(['Histogram of Residuals for maximal modulation of ' num2str(maxmod(i))])
end

%% Bootstrapping Data for IQR, +/- Ratio, Median of + Modulated Cells, Skewness

bootstrpstat = cell(1,length(maxmod));
bootcistat = cell(1,length(maxmod));
...
g = cell(1,length(maxmod)); %for number in each bin
l = cell(1,length(maxmod)); %for bin location
h = cell(1,length(maxmod)); %handle for bar/histogram object
b = cell(1,length(maxmod)); %handle for barchart
...
switch length(maxmod)
    case 4 % if there are four inputs (for V1 data)
        ...
        bar(1:length(maxmod),[median(bootstrpstat{1}(:,2))  median(bootstrpstat{2}(:,2)) median(bootstrpstat{3}(:,2))...
            median(bootstrpstat{4}(:,2))])
        errorbar(1:length(maxmod),[median(bootstrpstat{1}(:,2))  median(bootstrpstat{2}(:,2)) median(bootstrpstat{3}(:,2))...
        ...
        bar(1:length(maxmod),[median(bootstrpstat{1}(:,2)) median(bootstrpstat{2}(:,2))...
        ...
        bar(1:length(maxmod),[median(bootstrpstat{1}(:,3))  median(bootstrpstat{2}(:,3)) median(bootstrpstat{3}(:,3))...
            median(bootstrpstat{4}(:,3))])
        errorbar(1:length(maxmod),[median(bootstrpstat{1}(:,3))  median(bootstrpstat{2}(:,3)) median(bootstrpstat{3}(:,3))...
        ...
        bar(1:length(maxmod),[median(bootstrpstat{1}(:,3)) median(bootstrpstat{2}(:,3))...
        ...
        title('Bootstrap Median of Positively Modulated Cells + Errorbars')
        bar(1:length(maxmod),[median(bootstrpstat{1}(:,4))  median(bootstrpstat{2}(:,4)) median(bootstrpstat{3}(:,4))...
            median(bootstrpstat{4}(:,4))])
        errorbar(1:length(maxmod),[median(bootstrpstat{1}(:,4))  median(bootstrpstat{2}(:,4)) median(bootstrpstat{3}(:,4))...
        ...
        title('Bootstrap Median of Positively Modulated Cells + CI')
        bar(1:length(maxmod),[median(bootstrpstat{1}(:,4)) median(bootstrpstat{2}(:,4))...
