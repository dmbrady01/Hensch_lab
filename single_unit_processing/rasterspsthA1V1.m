%Rasters and PSTHs for A1 and V1
%
%Similar to rasterspsthpaper2 but does not include both trials
%
%Written by D.M. Brady 5/2010
...
subplot(10,2,[11,13,15])
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'inches');
set(gcf, 'PaperPosition', [.25 2.5 4 3])
hold on
...
rectangle('Position',[-200,numberoftrials,1400,numberoftrials],'FaceColor',[200/255 200/255 1]); %Light Blue
...
for i = 1:length(timestamps.aud)
    ...
    for j = 1:length(rasters.aud{i})
        line([rasters.aud{i}(j) rasters.aud{i}(j)], [(numberoftrials)+i-1 (numberoftrials)+i]);
    end
end
...
rectangle('Position',[-200,0,1400,gaud+5],'FaceColor',[200/255 200/255 1]); %Light Blue [1 220/255 220/255]); %Light Pink
...
plot(centers,spike_count.aud,'Color',[0 0 0],'LineWidth',1);
plot(centers,spike_count.blank,'b--','LineWidth',1);

%plot(centers,spike_count.both,'Color',[0 0 0],'LineWidth',1)
%plot(centers,spike_count.blank,'b--','LineWidth',1);

stimulus_start = line([0 0],[0 gaud+5]); %Creates a line indicating when stimulus started
set(stimulus_start,'Color','k','LineStyle','-','LineWidth',1)
stimulus_end = line([window.stim window.stim],[0 gaud+5]); %Creates a line indicating when stimulus ended
...
%saveas(rasters_figure, '3510 - pen7ch9 - A1 Aud New','pdf')