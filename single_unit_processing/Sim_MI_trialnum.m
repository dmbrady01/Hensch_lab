%Simulation of Multisensory Integration Data - Change Trial Number
%
...
ntrials = [5 20 50]; %number of trials in our simulation
lambda = 1; %different firing rates 
%V2(NgR, DR, LR: 3.0593 3.3462 6.2371); V1(NgR, DR, LR, OGAD: 3.0241 5.6185 9.9528 11.4759)

%%%%%%%%%%%%%%%%%%%%%%%%%%% RANDOM NUMBER GENERATOR %%%%%%%%%%%%%%%%%%%%%%
s = RandStream('swb2712','Seed',1322); %Starts a random number generator at a particular seed
RandStream.setDefaultStream(s)

%% Preparing modulation

%%%%%%%%%%%%%%%%%%%%%%%%%% PREPARING MODULATION %%%%%%%%%%%%%%%%%%%%%%%%%%
mod = cell(1,length(ntrials));   
...
%figure(3),scatter(mod{1},mod{1}); %if you would like to look at the distribution of modulation

%% Generating Average Firing Rate of Neurons
%Assuming neurons have poisson-like behavior

SM = cell(1,length(ntrials)); %Single stimulus condition
theoCM = cell(1,length(ntrials)); %Theoritical combined condition
CM = cell(1,length(ntrials)); %'Measured' combined condition
...
parfor i = 1:length(ntrials)
    SM{i} = mean(poissrnd(lambda,nneurons,ntrials(i)),2); %mean of ntrials in response to the single stimulus at a firing rate of lambda
    ...
    for j = 1:length(mod{i})
        CM{i}(j) = mean(poissrnd(lambda.*mod{i}(j),1,ntrials(i)),2); 
    end
end
...
MI = cell(1,length(ntrials)); %'Measured' MI
theoMI = cell(1,length(ntrials)); %'Theoritcal' MI
...
y_min = ones(length(ntrials),1);
y_max = ones(length(ntrials),1);
for i = 1:length(ntrials) %Figures out the max and min y values for MI under each lambda condition
...
for i = 1:length(ntrials)
    subplot(length(ntrials),3,3*i-2)
    hold on
    ...
    text(0,.75*y_width(2),['# of trials ' num2str(ntrials(i))]) %label of firing rate
    switch i
        case round(median(find(ntrials>0)))
...
y_max = ones(length(ntrials),1);
for i = 1:length(ntrials)
    y_max(i) = max(MInoinf{i}-theoMInoinf{i}); %makes sure to only get a finite max number
...
for i = 1:length(ntrials)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%  PLOTTING RESIDUALS %%%%%%%%%%%%%%%%%%%%%
    subplot(length(ntrials),3,3*i-1)
    hold on
    ...
    %%%%%%%%%%%%%%%%%%%%%%%  HIST OF RESIDUALS %%%%%%%%%%%%%%%%%%%%%
    subplot(length(ntrials),3,3*i)
    hold on
    hist(MInoinf{i}-theoMInoinf{i},30)
    ylim = get(gca,'YLim');
    line([0 0],[0 max(ylim)])
    title(['Histogram of Residuals for ' num2str(ntrials(i)) ' trials'])
end

%% Bootstrapping Data for IQR, +/- Ratio, Median of + Modulated Cells, Skewness

bootstrpstat = cell(1,length(ntrials));
bootcistat = cell(1,length(ntrials));
...
g = cell(1,length(ntrials)); %for number in each bin
l = cell(1,length(ntrials)); %for bin location
h = cell(1,length(ntrials)); %handle for bar/histogram object
b = cell(1,length(ntrials)); %handle for barchart
...
switch length(ntrials)
    case 4 % if there are four inputs (for V1 data)
        ...
        bar(1:length(ntrials),[median(bootstrpstat{1}(:,2))  median(bootstrpstat{2}(:,2)) median(bootstrpstat{3}(:,2))...
            median(bootstrpstat{4}(:,2))])
        errorbar(1:length(ntrials),[median(bootstrpstat{1}(:,2))  median(bootstrpstat{2}(:,2)) median(bootstrpstat{3}(:,2))...
        ...
        bar(1:length(ntrials),[median(bootstrpstat{1}(:,2)) median(bootstrpstat{2}(:,2))...
        ...
        bar(1:length(ntrials),[median(bootstrpstat{1}(:,3))  median(bootstrpstat{2}(:,3)) median(bootstrpstat{3}(:,3))...
            median(bootstrpstat{4}(:,3))])
        errorbar(1:length(ntrials),[median(bootstrpstat{1}(:,3))  median(bootstrpstat{2}(:,3)) median(bootstrpstat{3}(:,3))...
        ...
        bar(1:length(ntrials),[median(bootstrpstat{1}(:,3)) median(bootstrpstat{2}(:,3))...
        ...
        title('Bootstrap Median of Positively Modulated Cells + Errorbars')
        bar(1:length(ntrials),[median(bootstrpstat{1}(:,4))  median(bootstrpstat{2}(:,4)) median(bootstrpstat{3}(:,4))...
            median(bootstrpstat{4}(:,4))])
        errorbar(1:length(ntrials),[median(bootstrpstat{1}(:,4))  median(bootstrpstat{2}(:,4)) median(bootstrpstat{3}(:,4))...
        ...
        title('Bootstrap Median of Positively Modulated Cells + CI')
        bar(1:length(ntrials),[median(bootstrpstat{1}(:,4)) median(bootstrpstat{2}(:,4))...
