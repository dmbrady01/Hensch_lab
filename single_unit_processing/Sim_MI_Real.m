%Simulation of Multisensory Integration Data - Realistic
%
%
...
nneurons = [10000 10000 10000]; %number of neurons in simulation (realistic = 100, other = 1000)
    %V2(NgR, DR, LR: 84 105 104); V1(NgR, DR, LR, OGAD: 71 118 127 66)
ntrials = 20; %number of trials in our simulation

lambda = [1.8421 2.2105 4.9]; %different firing rates 
...
parfor i = 1:length(lambda)
    SM{i} = mean(poissrnd(lambda(i),nneurons(i),ntrials),2); %mean of ntrials in response to the single stimulus at a firing rate of lambda
